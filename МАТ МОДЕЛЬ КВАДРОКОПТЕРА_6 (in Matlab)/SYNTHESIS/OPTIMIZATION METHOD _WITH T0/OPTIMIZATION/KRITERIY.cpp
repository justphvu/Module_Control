...
double FCT(double K[])
{
	...
	{				
		...
		//J=J+1.E+1*pow(fabs(U[0])/(30/57.3),2.0);
		if (TIME>T0)
			//J=J+1.E+3*pow(X[1]-YDES,2.0)+2.E+2*pow(X[2],2.0);//для TETd и GAMd
			J=J+1.E+1*pow(X[1]-YDES,2.0);//+2.E00*pow(X[2],2.0);
		if (J!=J)
			J=1.E+9;
	}while (TIME<TF);
	J=J+15.E+1*T0*T0;//для TETd и GAMd
	J=J+15.E+1*T0*T0;
//	static int count=0;
//	if (count==0)
