...
function C_reduce = jacobian_C( state_eq, control_eq)
...
%---- size of the Jacobian matrix ----
 row = 1 : 5;   % array corresponding to rows of the Jacobian matrix
%                % y = [ V, THETA, Wz, H, DV ]
%row=1; %y=[DV];
...
for j = 1 : no_col
    ...
    for i = 1 : no_row
        tol = 0.1;
        a0 = ydx( state_eq, control_eq, row(i), column(j), dv0 );
        a1 = ydx( state_eq, control_eq, row(i), column(j), dv1 );
        a2 = ydx( state_eq, control_eq, row(i), column(j), dv2 );
        ...
        while k <= 100 
            ...
            if new_dv <= min_dv * state_eq( column(j) )
                ...
                a2 = a1;
                a1 = a0;
                a0 = ydx( state_eq, control_eq, row(i), column(j), new_dv );
                b1 = b0;
...
%-----------------------------------------
    % state = [ V; THETA; Wz; TET; H; deltaT; deltaV; PSIk; Wx; Wy; PSI; deltaN; deltaE];
...
answer( :, 3 ) = answer_C( :, 6 );
answer( :, 4 ) = answer_C( :, 9 );
answer( :, 5 ) = answer_C( :, 10 );
answer( :, 6 ) = answer_C( :, 11 );
%% Боковое движение
% answer( :, 7 ) = answer_C( :, 3 );
% answer( :, 8 ) = answer_C( :, 4 );
% answer( :, 9 ) = answer_C( :, 5 );
% answer( :, 10 ) = answer_C( :, 7 );
% answer( :, 11 ) = answer_C( :, 8 );
% answer( :, 12 ) = answer_C( :, 12 );
